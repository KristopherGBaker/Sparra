...
${color.bold("Commands")}
  ...
  role run --kind <r> [--backend b] [--model m] [--effort low|medium|high|xhigh|max] [--brief f|--brief-text s] [--contract f] [--prior-critique f]… [--holdout f] [--out f] [--workspace d] [--budget <usd>] [--verify] [--worktree [--keep-worktree]] [--unit-worktree <name>] [--expected-head <sha>] [--eval-base <ref>] [--baseline-command <cmd>]
                                                run ONE role once on a chosen backend (holdout wall enforced) — the cross-model seam (--budget overrides build.maxBudgetUsdPerItem, 0 = unlimited; --verify lets an in-place generator auto-run build.verifyCommands; --worktree runs an evaluator/reviewer/contract-evaluator in a temp WIP-snapshot worktree, torn down after — --keep-worktree retains it; --unit-worktree <name> runs the GENERATOR in a PERSISTENT named per-unit worktree reused across rounds — tear down with 'role rm-worktree'; repeatable --prior-critique inlines prior-round critique files into a contract-evaluator re-critique, .sparra/ paths OK; judge roles: --expected-head <sha> verifies the graded HEAD before launch, --eval-base <ref> scopes the changed-files judgment to <ref>..HEAD + WIP; evaluator only: --baseline-command <cmd> runs the allowlisted command at --eval-base's SHA in a throwaway worktree, injecting a runner-owned [VERIFIED BASELINE] the evaluator trusts over prose carveouts)
  ...
  eval [dir] [--contract f] [--backend b] [--model m] [--effort x] [--holdout f] [--out f] [--budget <usd>] [--worktree [--keep-worktree]] [--expected-head <sha>] [--eval-base <ref>] [--baseline-command <cmd>]

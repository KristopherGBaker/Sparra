...
${color.bold("Commands")}
  ...
  conduct --status <runId> [--json]             ZERO-SPEND, read-only projection of .sparra/conduct/<runId>/run.json: a header (runId, status, brain, decisions, timestamps, one-line prompt, landedInto when landed), one line per unit (id, title, outcome, score, cost, branch, short committedSha, mergedInto), and any still-parked decisions (seq + question + a 'conduct --decide <runId> <seq> <answer>' hint). --json emits the run.json fields + a pendingDecisions array. Paths/metadata only — never brief/contract/verdict contents. Unknown/unsafe runId → exit 1, no side effects

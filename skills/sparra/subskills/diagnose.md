...
| **Generator reports it couldn't run `tsc`/`npm test`, "verified by inspection" — ships uncompiled/untested code** | The writer's Bash wasn't auto-approved (permission mode doesn't auto-accept Bash), so it wrote blind | The generator now **self-verifies** on a worktree boundary: it auto-runs `build.verifyCommands` (typecheck/test/build) and fixes what it broke. If it still can't, check you're on a worktree (`git.strategy: worktree`) and that the command starts with a `verifyCommands` entry and has no chaining/redirect/network/commit (those are disqualified). Add the project's command to `build.verifyCommands`. (Codex confines these to its sandbox; for Claude they run unsandboxed like the evaluator's exercise.) |

...
| **Interactive `run_role`/`/sparra-loop`: a generator returns `ok: false` with EMPTY result text, but `emptyCompletion: true` and `filesChanged > 0`** | The work fully LANDED on disk; only the completion report failed to emit (empty completion or a budget-cap death at the report boundary) | NOT a failure and NOT "re-run the item" — a second generator would clobber the landed work (the fallback chain already refuses to). RESUME the session (`resumeSessionId`/`resumeBackend` = the result's `sessionId`/`backend`) to re-emit the report, or verify the tree yourself (typecheck/test) and accept the landed work. |

...
4. **Exercise & grade** — the evaluator is adversarial and **actually runs the artifact** — it does not read diffs. The [exerciser is pluggable](#exercisers). It marks every contract assertion PASS/FAIL **with evidence**, scores the **rubric**, and writes a structured verdict to `.sparra/verdicts/`. The rubric it sees is **anchored**: each criterion carries a one-line definition plus a generic band scale (**90+** exemplary · **70-89** solid, minor issues · **50-69** notable gaps · **<50** broken/deficient), so scores are grounded rather than free-floating. The **functionality score is also anchored to the assertion outcomes by construction** (`rubric.anchorFunctionality`, default on): when any assertion FAILED, the harness caps functionality at `round(100 × passed/total)` before recomputing the weighted total — a conservative **ceiling only** (it never raises a score; no assertions listed → no cap), with the applied cap noted in the verdict. Grading is against the **contract + rubric**, judged on **product impact** — a working, plan-satisfying artifact isn't failed over incidental contract nits, but an assertion satisfied by a **degenerate/gamed input** (a combine step fed identical copies, a structurally-wrong fixture, a stub) is a FAIL, not a pass-with-notes. The evaluator runs the **shipped/contracted verification as-is** — if an item's own committed tests or contracted verify commands crash as delivered, it won't be laundered into a pass by hand-correcting the invocation. Existing projects also run the repo's own test suite; new failures are a hard fail. On a failing round, the verdict flows back to the generator as **evidence-carrying feedback**: the blocking issues plus a `#<id>: <evidence>` line for every FAILED assertion (per-assertion evidence capped ~300 chars with a truncation marker; passed assertions are omitted), so the next round sees *why* it failed and *what was observed* — rendered purely from the holdout-redacted verdict (`src/build/feedback.ts`), never from raw evaluator session text. Pivot and blocked rounds carry the same per-assertion detail inside their own framing.
   - **Flakiness rerun gate.** After a PASSING verdict the harness (no model) re-runs the contract's verify commands `build.flakinessReruns` times (default 2, 0 = off) with the same safe executor. **Any rerun failure prevents a clean pass**: mixed exits demote the round as **flaky**, deterministic nonzero across all reruns as **failing-as-shipped** — the demotion carries the command + capped output back as blocking (holdout-redacted) feedback. Only all-runs-exit-0 keeps the pass; rerun-to-green never ships.
